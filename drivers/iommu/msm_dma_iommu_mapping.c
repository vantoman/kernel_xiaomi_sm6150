...
static void msm_iommu_map_free(struct msm_iommu_map *map)
{
	...
	/* Skip additional cache maintenance on the dma unmap path */
	if (!(map->attrs & DMA_ATTR_SKIP_CPU_SYNC)) {
		if (map->attrs && map->do_cpu_sync)
			dma_sync_sg_for_cpu(map->dev, map->sgl, map->nents,
					    map->dir);
			 ...
			 unsigned long attrs)
{
	...
	if (map) {
		if (nents == map->nents && dir == map->dir &&
		    (attrs & ~DMA_ATTR_SKIP_CPU_SYNC) ==
		    (map->attrs & ~DMA_ATTR_SKIP_CPU_SYNC) &&
		    sg_phys(sg) == map->buf_start_addr) {
			struct scatterlist *map_sg, *sg_tmp = sg;
